/*
 * AES-128 实现（VAES 变体）：与 aes128_ni.c 相同 API，额外在 ECB 中对 ≥32 字节段使用
 * 256 位 VAES 一次处理两个块，轮密钥预广播为 rk2[11]。
 *
 * 与 aes128_ni.c 二选一链接；勿同时链接。CPU 须支持 VAES（及部分机器上解密可能比
 * 纯 XMM 交错慢，见 aes128_ni.c 说明）。
 *
 * 编译：gcc -O2 -maes -msse2 -mavx2 -mvaes -c aes128_ni_vaes.c
 * MSVC: 启用 AES + AVX2 等（视工具链而定）。
 */
...
#include <immintrin.h>
#else
#error "aes128_ni_vaes.c: 仅支持带 WMmintrinsics 的 x86 编译器（GCC/Clang/MSVC）"
#endif
...
void aes128_encrypt_ecb(const AES128CTX *ctx, uint8_t *dst, const uint8_t *src, size_t nbytes)
{
    if (nbytes >= 32u) {
        ...
        int j;
        for (j = 0; j < 11; j++)
            rk2[j] = _mm256_broadcastsi128_si256(
                _mm_load_si128((const __m128i *)(const void *)(ctx->ni_rk + j * 16)));
        while (nbytes >= 32u) {
            __m256i s = _mm256_loadu_si256((const __m256i *)(const void *)src);
            s = _mm256_xor_si256(s, rk2[0]);
            for (j = 1; j <= 9; j++)
                s = _mm256_aesenc_epi128(s, rk2[j]);
            s = _mm256_aesenclast_epi128(s, rk2[10]);
            _mm256_storeu_si256((__m256i *)(void *)dst, s);
            src += 32;
            dst += 32;
            nbytes -= 32u;
        }
...
void aes128_decrypt_ecb(const AES128CTX *ctx, uint8_t *dst, const uint8_t *src, size_t nbytes)
{
    if (nbytes >= 32u) {
        ...
        while (nbytes >= 32u) {
            ...
            s = _mm256_xor_si256(s, rk2[0]);
            for (j = 1; j <= 9; j++)
                s = _mm256_aesdec_epi128(s, rk2[j]);
            s = _mm256_aesdeclast_epi128(s, rk2[10]);
            _mm256_storeu_si256((__m256i *)(void *)dst, s);
            src += 32;
            dst += 32;
            nbytes -= 32u;
